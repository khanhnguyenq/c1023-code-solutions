...
export function Todos() {
  ...
  async function toggleCompleted(todo: Todo) {
    ...
    try {
      const res = await fetch(`/api/todos/${todo.todoId}`, options);
      if (!res.ok) throw new Error(`${res.status}`);
      const toggled = (await res.json()) as Todo;
      const checkedArray = todos.map((i) =>
        i.todoId === toggled.todoId ? toggled : i
      );
